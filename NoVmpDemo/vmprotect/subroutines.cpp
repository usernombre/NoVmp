...
namespace vmp
{
	...
	std::pair<int, vm_parameter> extract_next_parameter( vm_state* state, const instruction_stream& is, int index )
	{
		vm_parameter out;
	...
	//
	void reduce_chunk( vm_state* vstate, instruction_stream& is, const std::vector<vm_parameter>& parameters )
	{
	...
	void update_handler_table( vm_state *vstate, const instruction_stream& is, int index )
	{
		...
		//
		int i_handler_table = is.next( X86_INS_LEA, { X86_OP_REG, X86_OP_MEM }, [ & ] ( const vtil::amd64::instruction& ins )
		{
			return
		...
		vstate->handler_table_rva = is[ i_handler_table ].address + is[ i_handler_table ].operands[ 1 ].mem.disp + 7;
		vtil::logger::log<CON_GRN>("HANDLER_TABLE = %p\n", vstate->handler_table_rva);
	}
